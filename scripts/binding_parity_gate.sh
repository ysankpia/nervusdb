...
set +e
run_step "parity-coverage-audit" bash scripts/parity_coverage_audit.sh
coverage_rc=$?
set -e
...
cat > "$JSON_FILE" <<JSON
{
  ...
  "checks": {
    ...
    "parity_coverage_audit": { "status": "${coverage_status}", "exit_code": ${coverage_rc} },
...
| scripts/parity_coverage_audit.sh | ${coverage_status} | ${coverage_rc} |
