...
**NervusDB v1.1.0 已经是功能完整、性能优秀的嵌入式图数据库。**
...
**行动建议**：从下周开始选择一个真实场景，用 NervusDB 构建实际应用。只有真实的使用，才能发现真正的问题。