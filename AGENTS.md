...
- A database file may have **at most one writer** at a time, across processes and within one process. Any number of **read-only** handles may coexist (they take a shared lock), and a writer excludes them. `NervusDb::open` must take an exclusive lock on `{path}` itself (`std::fs::File::try_lock`, stable since Rust 1.89 — never add a third-party dependency or a sidecar `.lock` file, which would break the two-file invariant). A contended open returns `GraphError::DatabaseLocked`.
- **A read-only handle must not be able to write, and that must be enforced in code, not by convention.** The shared lock does not prevent writes — it only excludes _other processes'_ writers (said so in `DbLock::acquire_shared`'s own doc comment, which relied on the caller to behave). Nothing checked it, so `with_transaction(|tx| tx.add_node(..))` on a read-only handle returned `Ok` and persisted: measured, WAL 0 → 12429 bytes and the node present after reopening. Worse, because shared locks do not exclude each other, N read-only handles became N **unmutexed writers**: two handles committing 500 transactions each returned `Ok` 1000 times and left **zero** of those writes behind, with `integrity_check` reporting no problem (it verifies graph structure, not whether writes landed). The guard therefore lives in `begin_transaction` — the single entry point `with_transaction` also goes through — not in `commit`, because by then the caller has already been told nothing is wrong.
