...
def test_order_by_multi_column():
    db.execute_write("CREATE (:MC {a: 1, b: 'z'})")
    db.execute_write("CREATE (:MC {a: 1, b: 'a'})")
    db.execute_write("CREATE (:MC {a: 2, b: 'm'})")
    rows = db.query("MATCH (n:MC) RETURN n.a, n.b ORDER BY n.a, n.b")
    assert_eq(len(rows), 3)
    assert_eq(rows[0]["n.b"], "a")
    assert_eq(rows[1]["n.b"], "z")
test("ORDER BY multi-column", test_order_by_multi_column)
...
def test_is_not_null_filter():
    db.execute_write("CREATE (:NL2 {name: 'has'})")
    db.execute_write("CREATE (:NL2 {})")
    rows = db.query("MATCH (n:NL2) WHERE n.name IS NOT NULL RETURN count(n) AS c")
    assert_eq(rows[0]["c"], 1)
test("IS NOT NULL filter", test_is_not_null_filter)
...
def test_abs():
    rows = db.query("RETURN abs(-7) AS v")
...
def test_remove_label():
    db.execute_write("CREATE (:RL:Extra {name: 'labeled'})")
    db.execute_write("MATCH (n:RL {name: 'labeled'}) REMOVE n:Extra")
    rows = db.query("MATCH (n:Extra) RETURN count(n) AS c")
    assert_eq(rows[0]["c"], 0)
test("REMOVE label", test_remove_label)
...
def test_param_string():
    rows = db.query("RETURN $greeting AS v", params={"greeting": "hello"})
    assert_eq(rows[0]["v"], "hello")
test("$param string", test_param_string)
...
def test_index_range_query():
    db_t, _ = fresh_db("idxops3")
    for i in range(50):
        db_t.execute_write(f"CREATE (:IX3 {{v: {i}}})")
    db_t.create_index("IX3", "v")
    rows = db_t.query("MATCH (n:IX3) WHERE n.v >= 40 RETURN n.v ORDER BY n.v")
    assert_eq(len(rows), 10)
    assert_eq(rows[0]["n.v"], 40)
    db_t.close()
test("index range query", test_index_range_query)
...
def test_syntax_error_detail():
    assert_throws(lambda: db.query("MATC (n) RETURN n"), nervusdb.SyntaxError)
test("syntax error detail", test_syntax_error_detail)

def test_unknown_function_error():
    assert_throws(lambda: db.query("RETURN noSuchFunction(1)"), nervusdb.SyntaxError)
test("unknown function error", test_unknown_function_error)

def test_delete_connected_node_error():
    db.execute_write("CREATE (:DN {v: 1})-[:R]->(:DN {v: 2})")
    msg = assert_throws(
        lambda: db.execute_write("MATCH (n:DN {v: 1}) DELETE n"),
        nervusdb.ExecutionError,
    )
    assert_true("DETACH DELETE" in msg or "delete" in msg.lower(), f"unexpected error: {msg}")
test("delete connected node error", test_delete_connected_node_error)
...
def test_snapshot_before_after_handles():
    db_t, db_path = fresh_db("concurrent2")
    db_t.execute_write("CREATE (:SH {v: 1})")
    before = db_t.query("MATCH (n:SH) RETURN count(n) AS c")
    db_t.execute_write("CREATE (:SH {v: 2})")
    db_t2 = nervusdb.Db(db_path)
    after = db_t2.query("MATCH (n:SH) RETURN count(n) AS c")
    assert_eq(before[0]["c"], 1)
    assert_eq(after[0]["c"], 2)
    db_t2.close()
    db_t.close()
test("snapshot isolation across handles", test_snapshot_before_after_handles)
