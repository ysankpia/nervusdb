...
#[test]
fn test_string_label_resilience() -> nervusdb_v2::Result<()> {
    // 1. Setup: Open a database in a temporary directory
    let dir = tempdir().map_err(nervusdb_v2::Error::Io)?;
    ...
    // Scope 2: Re-open and verify
    {
        ...
        match alice_name {
            nervusdb_v2::PropertyValue::String(s) => assert_eq!(s, "Alice"),
            _ => panic!("Alice name mismatch"),
        }
        match bob_name {
            nervusdb_v2::PropertyValue::String(s) => assert_eq!(s, "Bob"),
