...
fn execute_rows(db: &Db, query: &str) -> nervusdb_v2::Result<Vec<Row>> {
    let snapshot = db.snapshot();
    let prepared = nervusdb_v2::query::prepare(query)?;
    prepared
        .execute_streaming(&snapshot, &Default::default())
        .map(|row| row.map_err(nervusdb_v2::Error::from))
...
#[test]
fn test_graph5_relationship_label_expression_matches_rel_type() -> nervusdb_v2::Result<()> {
    let dir = tempdir()?;
...
#[test]
fn test_graph5_null_label_expression_keeps_unquoted_column_alias() -> nervusdb_v2::Result<()> {
    let dir = tempdir()?;
