...
fn t324_scoped_update() {
    ...
    let q = prepare(
        "MATCH (u:User) \
         FOREACH (tag IN ['A', 'B'] | SET u.last_tag = tag)" 
         // Note: Real usage often involves append, but M3 might not have list append in SET easily yet, 
...
fn t324_nested_foreach() {
    ...
    {
        let mut txn = db.begin_write();
        let count = q.execute_write(&db.snapshot(), &mut txn, &Params::new())
            .unwrap();
        txn.commit().unwrap();
        assert_eq!(count, 4);
    }
    ...
    {
        let snapshot = db.snapshot();
        let rows: Vec<_> = prepare("MATCH (n:R) RETURN sum(n.v) as total").unwrap()
            .execute_streaming(&snapshot, &Params::new())
            .collect::<Result<Vec<_>>>()
            .unwrap();
        // 3+4+12+13 = 32
