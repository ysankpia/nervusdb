...
#[test]
fn test_optional_match_on_empty_graph_returns_single_null_row() -> nervusdb_v2::Result<()> {
    let dir = tempdir()?;
    let db = Db::open(dir.path())?;
    let snapshot = db.snapshot();

    let query = nervusdb_v2::query::prepare("OPTIONAL MATCH (n) RETURN n")?;
...
#[test]
fn test_optional_match_with_bound_relationship_avoids_cartesian_expansion()
-> nervusdb_v2::Result<()> {
    let dir = tempdir()?;
    ...
    let snapshot = db.snapshot();
    let query = nervusdb_v2::query::prepare(
...
#[test]
fn test_optional_match_respects_labels_on_optional_end_node() -> nervusdb_v2::Result<()> {
    let dir = tempdir()?;
    let db = Db::open(dir.path())?;

    {
        let parsed = nervusdb_v2::query::parse(
            "CREATE (a1:X {id: 1}), (a2:X {id: 2}), (b1:Y), (b2:Y:Z), \
             (a1)-[:REL]->(b1), (a1)-[:REL]->(b2), (a2)-[:REL]->(b1)",
        )?;
        let mut saw_yz = false;
        for clause in parsed.clauses {
            if let nervusdb_v2::query::ast::Clause::Create(create) = clause {
                for pattern in create.patterns {
                    for element in pattern.elements {
                        if let nervusdb_v2::query::ast::PathElement::Node(node) = element
        ...
        let mut txn = db.begin_write();
        let setup = nervusdb_v2::query::prepare(
            "CREATE (a1:X {id: 1}), (a2:X {id: 2}), (b1:Y), (b2:Y:Z), \
             (a1)-[:REL]->(b1), (a1)-[:REL]->(b2), (a2)-[:REL]->(b1)",
        )?;
