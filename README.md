...
### 贡献政策：**仅接受 Issue，不接受外部代码**

本项目**不接受外部代码合并**。双授权成立的前提是单一主体持有全部版权：一旦合入他人的补丁，
那段代码就只能以 AGPL 授权、无法再用于商业许可，整个项目的双授权模式会因此失效。
本项目选择不收集 CLA 来规避这层摩擦，代价就是——**外部 PR 一律不受理**（由工作流自动关闭）。

这是授权层面的考虑，与你的代码质量无关：一个补丁可以写得很好，但在这里依然无法合入。

**欢迎的东西**（这些有实际价值，且会被真正处理）：

- **Bug 报告**：附上你执行的命令、原始输出与最小复现。可复现是这里唯一的硬通货。
- **对文档数字的更正**：`benches/` 里的基准会打印自己的配置，任何数字都可被验证；
  能证明某个数字是错的，就是实打实的贡献。
- **设计讨论**：撞上 [ROADMAP.md](ROADMAP.md) 里的限制，或不同意某个设计决定，
  开 Issue 说明实际场景——这会直接影响后续做什么。
- **安全问题**：请私信 **luhuizhx@gmail.com**，不要开公开 Issue。

**想自己改代码？** AGPL 本来就赋予你这个权利，不需要任何许可：内部使用、自己改自己用，
都没有义务；只有「修改后对外提供网络服务」才需要公开你的改动。
个人 fork 做实验完全没问题，只是不会回流到本仓库。

[CLA.md](CLA.md) 仍然存在，但**仅适用于维护者明确邀请的改动**；自行提交 PR 不构成对该协议的接受。

**关于提交者身份**：本仓库目前只有维护者一人拥有写权限，main 分支已启用保护
（禁止强制推送，要求 CI 通过）。配合自动关闭外部 PR 的工作流，他人无法向 main 提交代码。
