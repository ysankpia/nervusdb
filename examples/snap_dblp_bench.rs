...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let dataset_path = resolve_dataset("com-dblp.ungraph.txt", "/data/datasets");
    ...
    let db_path = target_dir.join("dblp_team3.db");
    ...
    println!("============================================================");
    println!(" [STAGE 1] SNAP com-DBLP REAL-WORLD GRAPH BENCHMARK");
    println!(" TEAM 3 (Pure DeepSeek Kernel Architecture)");
    println!(" Dataset  : {}", dataset_path.display());
    println!(" Target DB: {} (Internal SSD)", db_path.display());
    println!(
    ...
    println!("-> Step 1: Loading & Analyzing SNAP DBLP Dataset...");
    let t_parse = Instant::now();
    let file = File::open(&dataset_path)?;
    let reader = BufReader::new(file);

    let mut raw_edges: Vec<(u32, u32)> = Vec::with_capacity(1_050_000);
    let mut degrees: HashMap<u32, usize> = HashMap::with_capacity(320_000);
    ...
    while node_idx < num_nodes {
        ...
        db.with_transaction(|tx| {
            for &raw_id in chunk {
                ...
                let eid = tx.add_node(HashSet::from(["Author".to_string()]), p)?;
    ...
    // 4. Edge Ingestion
    println!(
        "-> Step 4: Ingesting {} real-world edges in batches...",
        num_edges
    );
    let t_edges = Instant::now();
    let edge_batch = 50_000usize;
    let mut edge_idx = 0;
        ...
        db.with_transaction(|tx| {
            for &(raw_u, raw_v) in chunk {
                ...
                if src != 0 && dst != 0 && src != dst {
                    tx.add_edge(src, dst, "COAUTHOR", HashMap::new(), 1.0)?;
                }
            }
            Ok(())
        })?;
    ...
    println!(
        "\n   => Edge Ingestion: {:.2?} | {:.0} ops/s",
        edge_dur, edge_ops
    );

    // 5. Checkpoint
    println!("-> Step 5: Checkpointing to Internal SSD...");
    ...
    println!("\n-> Step 6: Query Benchmark on Real Supernodes (Hub Authors)...");
    ...
    // 6.2: 2-Hop on Top 50 Hubs (Co-author of Co-authors)
