...
Issues and specs for this repo live as GitHub issues on `ysankpia/nervusdb`. Use the
...
## This repo's issue policy

Issues are the **only** accepted inbound channel. `CONTRIBUTING.md` and AGENTS.md §5.2
state that external pull requests are refused by policy (dual licensing), and
`.github/workflows/close-external-prs.yml` closes them automatically. Work that comes
from an issue is implemented by the maintainer on a branch and merged via PR.

That is why the flag below is permanently `no`: a PR is _not_ a valid way to request a
change here, so triaging one as if it were would invite work that cannot be merged.

## Pull requests as a triage surface

**PRs as a request surface: no.** _(Do not set to `yes` for this repo — see above.)_
...
## Pull requests this repo does open

The maintainer's own work goes through a PR onto the protected `main`, per AGENTS.md
§5.1 — five required checks (`Test (ubuntu-latest)`, `Test (macos-latest)`,
`SDK (node)`, `SDK (python)`, `Docs build`), `strict` on. Merges are squash merges with
branch deletion. This is the maintainer path, not a triage surface.
...
- **Blocking**: GitHub's **native issue dependencies** — the canonical, UI-visible
  ...
  gate). Where dependencies aren't available, fall back to a `Blocked by: #<n>, #<n>`
  line at the top of the child body. A ticket is unblocked when every blocker is closed.
- **Frontier query**: list the map's open children (`gh issue list --state open`, scoped
  to the map's sub-issues / task list), drop any with an open blocker
