...
// 登录方法直接或间接调用的所有方法（两跳）
const called = db
  .find({ subject: 'method:User#login', predicate: 'CALLS' })
  .follow('CALLS')
  .all();

// 反向联想：谁调用了 issueToken？
const callers = db
  .find({ object: 'method:Auth#issueToken' })
  .followReverse('CALLS')
  .all();
```
...
function addEdge(s: number, pLabel: string, o: number) {
  edges.push({ data: { id: `${s}-${pLabel}-${o}`, source: String(s), target: String(o), label: pLabel } });
}
...
const args = [
  'auto-compact', 'codegraph.synapsedb',
  '--mode=incremental', '--orders=SPO', '--min-merge=2', '--hot-threshold=1', '--max-primary=1', '--auto-gc'
];
