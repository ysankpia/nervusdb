...
- 结构化过滤：`whereProperty(name, op, value, target)` / `whereLabel(labels, { on, mode })`
...
- `variablePath()` 定义变长路径（同步物化）；`variablePathStream()` 提供真流式 BFS
...
const consistent = await db.withSnapshot(async (snap) =>
  // 注意：withSnapshot 回调内，为保证快照语义，find() 将回落为“即刻物化”模式
...
> 注：`find()` 默认返回“惰性链”，仅在 `withSnapshot` 回调内回落为“即刻物化”以保证读一致性。
