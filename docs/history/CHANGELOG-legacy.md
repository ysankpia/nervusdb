# Changelog — the GraphLite line (archived)

This file holds the release history from when the project was called **GraphLite**,
through `v1.0.0` and its release candidates. The project is now **NervusDB**; the
current release history is in the repository-root `CHANGELOG.md`.

It is kept for the reasoning it records — why the format froze at v4, which defects
were found, and how. The engine is the same one; only the name changed, and the
on-disk magic with it (`GLDB` → `NVDB`, format version 5).

One thing to avoid confusing: an unrelated earlier attempt by the same author also
used the name `nervusdb`. It used a different engine and a different storage format,
and is archived on GitHub as `nervusdb-legacy`. **Its releases are not described
here.**
