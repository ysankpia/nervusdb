...
- [ ] 稳定的公开 Rust API（`nervusdb` facade + `nervusdb-query`）
...
1. **API 稳定性**：确保 `nervusdb` 和 `nervusdb-query` 公开 API 稳定
...
T57.3 - API 稳定性审查 (1d)
  ├─ 审查 `nervusdb` facade API
  ├─ 审查 `nervusdb-query` API
  └─ 修复不一致/临时 API
...
- 扩展 `nervusdb-query/tests/create_test.rs`
- 补充边界测试（空值、大值、特殊字符）

**Day 3**: WHERE 测试
- 扩展 `nervusdb-query/tests/filter_test.rs`
- 补充比较/逻辑运算测试
...
- 新增 `nervusdb-storage/tests/tombstone_semantics.rs`
- 扩展 `nervusdb-storage/tests/m1_graph.rs`

**Day 7**: Compaction 语义测试
- 扩展 `nervusdb-storage/tests/m2_compaction.rs`
...
每个新增 API 必须有对应测试：
- `#[test]` 函数
- 测试文件位于 `nervusdb-*/tests/` 或 `src/*/tests/`

### 6.2 集成测试

- `nervusdb-query/tests/` 端到端测试
- 覆盖完整 query → storage → replay 链路

### 6.3 Crash Gate

- 运行 `cargo test -p nervusdb-crash-test`
- 验证 100+ 次 crash 恢复
...
- `docs/spec.md` 6.3 章节
- `nervusdb-query/tests/create_test.rs`
- `nervusdb-storage/tests/m1_graph.rs`
- `nervusdb-storage/tests/m2_compaction.rs`
