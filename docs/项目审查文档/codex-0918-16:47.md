SynapseDB 项目审查报告（codex / ultrathink）
...
- WAL v2 批次与重放（BEGIN/COMMIT/ABORT，durable commit）
  ...
  - tests/wal_*.test.ts 多组覆盖：abort/durable/txId 幂等/尾部截断
...
里程碑 B（观测与治理）
4) CLI 观测补强：db:readers、db:hot --topN、db:stats 增强维度与耗时打印；auto-compact 输出决策因子与分数。
5) 读者注册优化：目录扫描节流 + 缓存，支持最大读者数阈值与预警。

里程碑 C（查询与性能）
6) 属性倒排原型：对高频属性键建立轻量倒排，where 可识别“可下推”条件以减少页读取。
7) 大页读写与 Rust/WASM 热点加速（评估项）。
