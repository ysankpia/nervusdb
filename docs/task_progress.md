...
| T7 | Node 绑定去插件化 + 修复 Cypher 调用致命 Bug | L3 | P0 | WIP | feat/T7-node-thin-binding | 删 `PluginManager`/JS 聚合/TS Cypher；Cypher 只走 Rust Core 执行器 |
| T8 | Temporal 变为 optional feature（Default OFF） | L3 | P0 | Plan | - | `nervusdb-core`/N-API 增加 `temporal` feature；默认关闭，按能力探测启用 |
| T9 | Node Tests 纳入 CI（覆盖 Binding ↔ Native） | L2 | P0 | Plan | - | CI 增加 node job：typecheck + tests（含 native path） |
